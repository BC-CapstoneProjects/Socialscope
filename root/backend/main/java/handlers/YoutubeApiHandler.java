...
private JSONObject formatQueryJSON(JSONObject responseData) {
	...
	try {
		...
		for (int i = 0; i < inPosts.length(); i++) {
			...
			//System.out.println(postData.put("text", currentPost.getJSONObject(0).getJSONObject("snippet").getJSONObject("localized").getString("description")));
